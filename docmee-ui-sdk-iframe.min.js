...
class J {
  ...
  constructor({
    ...
    this.onMessage = i, this.container = n, location.protocol.startsWith("file") && console.log(
      ...
      "🔴 No se puede ejecutar bajo el protocolo 'file', ¡por favor inicie un servicio http para ejecutar! 🔴 "
    ), r || console.log(
      "%c Al inicializar, ¡el token no puede estar vacío!",
      "color: #d7514f; background-color: #2e2e2e"
    ), s && (j.BASE_URL = s), this.init({ token: r, page: t, pptId: a, ...o });
  }
  _postMessage(r) {
    var t, n;
    if (!this.iframe.contentWindow) throw new Error("¡iframe no montado!");
    (n = (t = this.iframe) == null ? void 0 : t.contentWindow) == null || n.postMessage(r, this.docmeeHref);
  }
  init({ token: r, page: t = "dashboard", ...n }) {
    if (t === "editor" && !n.pptId)
      throw new Error("Al inicializar la página del editor, ¡debe pasarse pptId!");
   ...
   * @param {string} latestToken Nuevo token
   */
  updateToken(r) {
    /(a|s)k_.+/.test(r) || console.error("¡Token incorrecto!"), this.token = r, this.query.token = r, this.iframeMounted && this._postMessage({
      type: "transParams",
      data: {
        token: r
      }
  ...
  navigate({ page: r, pptId: t, templateId: n }) {
    if (!$[r]) throw new Error(`La página ${r} no existe`);
    this._postMessage({
      type: "nav",
      data: {
        page: r,
